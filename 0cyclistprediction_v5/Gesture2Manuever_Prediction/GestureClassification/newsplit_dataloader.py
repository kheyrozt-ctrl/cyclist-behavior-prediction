...
def create_and_save_dataloaders(data_dir, dataset_type, fold_number, batch_size=32):
    ...
    print(f"Creating DataLoader for fold {fold_number} ({branch})...")
    paths = {}
    ...
    feature_dim = int(next(iter(loaders["train"]))[0].shape[-1])
    loaders["mean"] = np.zeros(feature_dim, dtype="float32")
    loaders["std"] = np.ones(feature_dim, dtype="float32")
