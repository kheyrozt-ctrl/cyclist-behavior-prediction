...
def train_all_folds(dataloaders_dir, model_save_dir, num_epochs=80, hidden_size=128,
                    learning_rate=0.001, num_layers=2, patience=5, eval_period=2):
    """Train standalone maneuver prediction models for all 5 folds."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    start_time = time.time()

    for fold_number in range(1, 6):
        with open(os.path.join(dataloaders_dir, f'dataloaders_fold_{fold_number}.pkl'), 'rb') as f:
        ...
        input_size = 2  # 2D trajectory (gesture weight features)
...
if __name__ == "__main__":
    dataloaders_dir = 'Gesture2Manuever_Prediction/ManueverPrediction/DataLoaders'
    model_save_dir = 'Gesture2Manuever_Prediction/ManueverPrediction/Models'
    train_all_folds(dataloaders_dir, model_save_dir)
