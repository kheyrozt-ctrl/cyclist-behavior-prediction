...
The reference allocation follows the Proposal: up to 10,000 A100 GPU-hours, four concurrent A100s, 10 TB cloud storage, and an estimated 2–4 TB for raw captures, processed keypoints, and renderings [S3]. These are capacity-planning values, not consumption measurements.
...
RealSense RGB or webcam (default 640 × 480 @ 30 fps)
  → one selected pose adapter (MediaPipe 33 points or trt_pose 18→33 mapping)
...
| Pose | Shared `mediapipe` or `trt` adapter | The 18→33 mapping remains a compatibility transform, not measured 33-joint data |
...
| Unity simulation telemetry is recorded at 20 Hz into `SimBike.csv`, `DynamicObjects.csv`, and `StaticObjects.csv` | Ahsan thesis, PDF pp. 32–33 | Source evidence |
| Video and simulator records are aligned using shared Unix timestamps | Ahsan thesis, PDF pp. 35–36 | Source evidence |
...
| RTX PRO 6000→A100→Jetson mapping; Holoscan/TensorRT/Isaac/Newton stack | Proposal, PDF pp. 1–3 | Proposal requirement |
...
| `proposal_cave_3d_context_v1`* | 60–120 Hz 3D skeletons synchronized with scene context [S3] | Reference cloud-training dataset under the declared submission assumptions |
...
```mermaid
flowchart LR
  subgraph CAVE["CAVE trust zone — RTX PRO 6000"]
  ...
  subgraph CLOUD["Cloud training zone — A100"]
  ...
  subgraph EDGE["Edge zone — Jetson AGX Orin"]
...
- never present duplicated/interpolated 18→33 joints as measured points;
...
| `T-CLOUD` | 3D skeleton plus scene context | Task-specific intention and, where enabled, trajectory | Reference 100–300M teacher/backbone family defined by the Proposal [S3] |
| `S-EDGE` | Exact distilled view declared by its signature | Same validated task semantics as its teacher | Validated ≤50M edge family defined by the Proposal [S3] |
...
- **[S4]** `cyclist_inference/README.md` and implementation — intersection inference architecture and present model/window behavior;
- **[S5]** `model_package/README.md` and implementation — bus-stop Stage-1/Stage-2 TorchScript package and present cold-start/device limitations;
- **[S6]** `pose_detection/README.md` and implementation — Jetson/RealSense/trt_pose environment and current TensorRT fallback behavior; and
...
| Data acquisition | Evidence for native 60–120 Hz 3D skeleton capture and synchronized context fields |
