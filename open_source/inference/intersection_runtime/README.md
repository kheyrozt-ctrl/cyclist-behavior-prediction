...
```bash
bash setup.sh
cd ~/mathias_ws/open_source/inference/intersection_runtime
...
|------|---------|-------------|
| `--explicit-model` | `../../training/intersection_intention_legacy/.../ExplicitModels/best_model_fold_5.pkl` | Explicit gesture model weights |
| `--implicit-model` | `../../training/intersection_intention_legacy/.../ImplicitModels/best_model_fold_5.pkl` | Implicit gesture model weights |
| `--maneuver-model` | `../../training/intersection_intention_legacy/.../ManueverPrediction/Models/best_model_fold_5.pkl` | Maneuver prediction model weights |
...
**Model not found**: Ensure the `open_source/training/intersection_intention_legacy/` directory is at `../../training/intersection_intention_legacy/` relative to this directory, or provide explicit paths with `--explicit-model`, `--implicit-model`, and `--maneuver-model`.

**trt_pose model not found**: Ensure `../../pose_backends/trt_pose/models/resnet18_baseline_att_224x224_A_epoch_249.pth` and `../../pose_backends/trt_pose/models/human_pose.json` exist. Run `bash ../../pose_backends/trt_pose/download_models.sh` if needed.

**trt_pose import error with `--pose mediapipe`**: The backends use lazy imports â€?trt_pose is only imported when `--pose trt` is selected, so MediaPipe mode works without trt_pose installed (and vice versa).

**TensorRT optimization fails (DECONVOLUTION weight count errors)**: This is a known issue with `fp16_mode=True` and deconvolution layers â€?FP16 conversion halves the weight count, causing a mismatch. The system automatically falls back to PyTorch CUDA inference, which is still fast on the Orin. To force FP32 TensorRT instead, change `fp16_mode=True` to `fp16_mode=False` in `open_source/pose_backends/trt_pose/pose_detect.py`. If a stale TRT cache exists from a previous attempt, delete `../../pose_backends/trt_pose/models/resnet18_baseline_att_224x224_A_trt.pth` before retrying.
